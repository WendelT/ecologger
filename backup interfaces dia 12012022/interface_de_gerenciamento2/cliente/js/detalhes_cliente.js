    ...
    function listUnits(callback)
{
        ...
        success: function (obj, textstatus) 
        {
            if( !('error' in obj) ) {
                ...
                for(i=0; i<obj.result.length; i++)
                {
                    ...
                    btn2.innerHTML = '<button type="button" data-toggle="modal" data-target="#addDatalogger"><img id="imgButton2" src="img\\properties.png"/></button>';
                    btn2.id = "buttonImg2";                    
    ...
    $(document).ready(function(){
        ...
        $('#insert_datalogger').on('submit', function(event){
            event.preventDefault();
            console.log($("#insert_datalogger"));
            //Receber os dados do formulário
            var dados = $("#insert_datalogger").serialize();
            console.log(dados);
            console.log("Entra porra");
            $.post("php/CUnidade.php", dados, function(retorna){
                if(retorna){

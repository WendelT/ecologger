        ...
        switch($_POST['functionname']) {
            ...
            case 'producaoUnidades':
                ...
                $positionQuery = "SELECT id, id_inv, dados_list as dados, infos_list as infos from tab_final t1 WHERE id IN
                                    ...
                                    id_unico FROM unidade_logger WHERE id_unidade='".$_POST['arguments']."')) GROUP BY id_inv)";
                ...
                $potenciaQuery = "SELECT id, id_inv, dados_list as dados, infos_list as infos from tab_final t1 WHERE id IN
                                    (SELECT MAX(id) FROM tab_final WHERE id_inv IN (SELECT id_inv FROM tab_final WHERE id_unico IN (SELECT 
                                    id_unico FROM unidade_logger WHERE id_unidade='".$_POST['arguments']."')) GROUP BY id_inv)";
                                    $consulta = mysqli_query($conn,$positionQuery);
                                    $data1 = array();
